...
| file | what for |
|---|---|
| `dpi/httpx/httpx.dpi` | the program dpid execs — a two-line launcher that runs `dist/main.js` with the absolute path of your `node`, because dpid inherits Dillo's PATH, not your shell's |
| `dpidrc` | `proto.httpx=httpx/httpx.dpi` — how the scheme reaches the plugin; created from the system file if you had none |
