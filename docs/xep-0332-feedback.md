...
1. `chunkedBase64` sequencing is underspecified. The `nr` attribute's
   ...
   idle timeout. The encoding of an empty body is undefined (we send one
   empty chunk with `last='true'`, since a receiver needs *some* final
...
3. The `jingle` mechanism's semantics are unclear. §"jingle" embeds a
   ...
   example embeds an RTP/ICE session, which cannot carry an HTTP body and
   is presumably a copy-paste from XEP-0166; and (b) the XEP never says
...
5. Error semantics are thin. The XEP does not say what a responder
   ...
   malformed `<req>` (we send an IQ `bad-request` error, because it never
   became an HTTP request and an HTTP status would be wrong). Proposal: a
   short "error conditions" section with this mapping.
...
- Responder reply timing for streamed request bodies. The responder can
  ...
  timeout must cover the whole transfer. Worth a sentence, since it
  surprises every HTTP-minded implementer.
...
- `Connection`-class headers: §Security says to ignore them; it should
  ...
  in both directions.
- The `httpx://` URI scheme: parsing `user@domain` authorities through
