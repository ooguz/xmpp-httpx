...
`examples/electron/` is what the WebExtension could not be: a browser where
`httpx://` is a **scheme Chromium itself fetches**, not a URL a page pretends to
navigate. `protocol.handle("httpx", …)` is the whole difference.
...
The proof is in one log line from a real run:
...
URL and fetched it through the protocol handler — which is precisely the work
...
- **Account JIDs are best-effort.** A localpart with characters a hostname
  cannot hold will not survive; hosts are also lowercased, which JID localparts
...
- Page content lives in `WebContentsView`s: `sandbox: true`,
  `nodeIntegration: false`, **no preload at all**.
...
**ESM throughout**, like the rest of the project. Two consequences: `__dirname`
is `import.meta.dirname`, and the preload is emitted as `preload.mjs`, because
Electron decides a preload's module type from its extension and an ESM preload
must be `.mjs` (with `sandbox: false`, which the chrome view already needs for
`contextBridge`). The example began as CommonJS and was converted when the root
`tsc` — correctly — refused `export` in a file it resolved as CJS under
`verbatimModuleSyntax`; the app was re-verified after the switch.

Keeping `protocol.ts` free of Electron is what makes the interesting half
testable: 17 cases cover the URL encoding round trip, the CSP a page cannot
loosen, method/header/body forwarding, status pass-through, the not-connected
page, an XMPP refusal mapped to 403, and error-page escaping — none of which
needs a window.
...
- The address bar shows `httpx://web@httpx.localhost/` — the decode works.
...
- **Packaging.** No `electron-builder` config, so nothing installs a `.desktop`
  entry or registry key — see [os-scheme-handlers.md](os-scheme-handlers.md) for
