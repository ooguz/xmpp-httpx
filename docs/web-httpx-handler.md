...
2. Redirect to the extension page: `ext+httpx://…` on Firefox, or the
   extension's own URL on Chromium — which requires knowing the extension ID
   and having `web_accessible_resources` expose `browser.html`, i.e. the
...
3. If the extension is not installed, explain what httpx is and link to the
   install page — which is arguably the most valuable thing it does, since a
